...
async def get_event_seats(event_id: str = Path(..., description="The event ID")):
    ...
    try:
        ...
        for item in result["items"]:
            ...
            price = float(price_value) if isinstance(price_value, Decimal) else price_value
                
            seats.append(EventSeatResponse(
    ...
    try:
        ...
        for item in seats_result["items"]:
            ...
            if seat_result["status"] == "error":
                ...
                print(f"Warning: Failed to create event seat {item.get('seat_pos')}: {seat_result['error']}")
                continue
