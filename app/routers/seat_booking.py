...
from app.models.event import PaymentConfirmRequest, BookingResponse, BookingCancelRequest
from app.database import db_client
from app.utils import (
    generate_booking_id, get_current_timestamp, 
    create_booking_transaction_items, create_cancellation_transaction_items,
    create_enhanced_booking_transaction_items, create_enhanced_cancellation_transaction_items
)
...
async def confirm_booking(
    ...
    try:
        ...
        if not payment_request or payment_request.payment_status not in ["successful", "failed"]:
            raise HTTPException(
                status_code=400,
        ...
        if payment_request.payment_status == "failed":
            raise HTTPException(
                status_code=400,
                detail="Payment failed. Booking not confirmed."
            )
        ...
        if holdings_result["count"] > 1:
            ...
            print(f"Warning: Multiple holdings found with ID {holding_id}, using the first one")
        ...
        if not event_id:
            raise HTTPException(
                status_code=500,
                detail="Holding record missing event_id"
            )
        if not user_id:
            raise HTTPException(
                status_code=500,
                detail="Holding record missing user_id"
            )
        if not seats:
            raise HTTPException(
                status_code=500,
                detail="Holding record missing seats"
            )
        ...
        if user_result["status"] == "not_found":
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Error validating user: {user_result['error']}"
            )
        ...
        transact_items = create_enhanced_booking_transaction_items(
            event_id, booking_id, holding_id, user_id, seats, payment_request.payment_status
        )
...
async def cancel_booking(
    ...
    try:
        ...
        if bookings_result["count"] > 1:
            # Log the issue and use the first one
            print(f"Warning: Multiple bookings found with ID {booking_id}, using the first one")
        ...
        if not event_id:
            raise HTTPException(
                status_code=500,
                detail="Booking record missing event_id"
            )
        if not user_id:
            raise HTTPException(
                status_code=500,
                detail="Booking record missing user_id"
            )
        if not seats:
            raise HTTPException(
                status_code=500,
                detail="Booking record missing seats"
            )
