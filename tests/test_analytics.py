...
def test_event_with_bookings():
    ...
    confirm_response1 = client.post(f"/{holding1['holding_id']}/confirm", json=confirm_data1)
    ...
    confirm_response2 = client.post(f"/{holding2['holding_id']}/confirm", json=confirm_data2)
...
class TestAnalytics:
    ...
    def test_get_event_analytics(self, test_event_with_bookings):
        ...
        expected_revenue = 1000.0 + 500.0 + 500.0  # A-1 (VIP) + B-1 (Standard) + B-2 (Standard)
    ...
    def test_get_seat_analytics_with_filters(self, test_event_with_bookings):
        ...
        # Filter by seat type
        response = client.get(f"/events/{event['event_id']}/seats/analytics?seat_type=VIP")
        assert response.status_code == 200
        ...
        response = client.get(f"/events/{event['event_id']}/seats/analytics?seat_state=booked")
        assert response.status_code == 200
    ...
    def test_get_booking_analytics_with_filters(self, test_event_with_bookings):
        ...
        # Filter by state
        response = client.get(f"/events/{event['event_id']}/bookings/analytics?state=confirmed")
        assert response.status_code == 200
    ...
    def test_analytics_with_cancelled_booking(self, test_event_with_bookings):
        ...
        cancel_response = client.post(f"/{bookings[0]['booking_id']}/cancel", json=cancel_data)
    ...
    def test_analytics_pagination(self, test_event_with_bookings):
        ...
        response = client.get(f"/events/{event['event_id']}/bookings/analytics?limit=1&offset=0")
        assert response.status_code == 200
        ...
        # Test with offset
        response = client.get(f"/events/{event['event_id']}/bookings/analytics?limit=1&offset=1")
        assert response.status_code == 200
    ...
    def test_analytics_error_handling(self, test_event_with_bookings):
        ...
        event = test_event_with_bookings["event"]
        response = client.get(f"/events/{event['event_id']}/seats/analytics?seat_state=invalid")
        assert response.status_code == 200  # Should return empty list
        
        response = client.get(f"/events/{event['event_id']}/bookings/analytics?state=invalid")
        assert response.status_code == 200  # Should return empty list
