...
def test_setup_with_booking():
    ...
    confirm_response = client.post(f"/{holding['holding_id']}/confirm", json=confirm_data)
...
class TestCancellationEdgeCases:
    ...
    def test_successful_cancellation(self, test_setup_with_booking):
        ...
        cancel_response = client.post(f"/{booking['booking_id']}/cancel", json=cancel_data)
        ...
        seats_response = client.get(f"/events/{test_setup_with_booking['event']['event_id']}/seats")
        assert seats_response.status_code == 200
    ...
    def test_cancellation_without_booking_id(self, test_setup_with_booking):
        ...
        assert cancel_response.status_code == 200  # Should still work as booking_id is in path
    ...
    def test_nonexistent_booking_cancellation(self, test_setup_with_booking):
        ...
        cancel_response = client.post("/nonexistent-booking-id/cancel", json=cancel_data)
    ...
    def test_double_cancellation_attempt(self, test_setup_with_booking):
        ...
        cancel_response1 = client.post(f"/{booking['booking_id']}/cancel", json=cancel_data)
        assert cancel_response1.status_code == 200
        
        # Second cancellation should fail
        cancel_response2 = client.post(f"/{booking['booking_id']}/cancel", json=cancel_data)
    ...
    def test_cancellation_with_different_booking_id_in_body(self, test_setup_with_booking):
    ...
    def test_cancellation_verification_after_success(self, test_setup_with_booking):
        ...
        hold_response = client.post(f"/events/{test_setup_with_booking['event']['event_id']}/hold", json=hold_data)
        assert hold_response.status_code == 200
